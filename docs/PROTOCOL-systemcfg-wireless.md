...
`radius_das_enabled` is modeled but rejected at the admin API while the
DAS/DAD client rows are blocked),
...
aaa.<n>.radius.das.status=enabled/.das.port=<3800+n>/radius.dad.status=enabled   OMITTED — BLOCKED (2026-09-19 acct lane, §12 row 1014): radius_das_enabled is modeled on the profile but rejected at the admin API; the status/port rows cannot ship without the client rows below (a client-less DAS block is not a jar shape), whose <ip> source is unrecovered from the jar
aaa.<n>.radius.dad.client.<i>.cidr=<ip>/32 + .secret=<x_secret>   OMITTED — BLOCKED (§12 row 1014): the dad client <ip> source is unrecovered from the jar; no candidate value is invented
aaa.<n>.radius.das.client=<ip> / .das.secret=<x_secret>       OMITTED — BLOCKED (§12 row 1014): the das client <ip> source is unrecovered from the jar
...
| `radius_das_enabled` (wpa-eap only) | radiusprofile `radius_das_enabled` (§12 row 1014) | REJECTED at the admin API while set: the DAS/DAD client rows are BLOCKED (client `<ip>` source unrecovered from the jar — no value invented); the renderer flags out-of-API carriers with a message-only Alert and emits no das/dad rows |
...
| `# mgmt` ledbar block | emitted headerless between `# users` and `# wlans` (String.txt:2566-2745) | omitted | ledbar falls back to firmware defaults. LED lane (2026-09-19) attempted the row-for-row recovery: the row keys are known (§3 item 4 of PROTOCOL-mgmt.md: ledbar.status/persistent/brightness/active/color.1.{color,r,g,b}) but the VALUES, per-row conditions and exact order live only in the javap dump of config_String, which the lane's sandboxed session could not read (nono `path_not_granted` on the main checkout's tmpwork). Byte-exact or absent — the block stays omitted until a session with javap access recovers config_String §2566–2745 verbatim. Live-proof obligations: the block's on-wire bytes in a captured full provisioning response, and real LED state changes per override on the bench AP |
...
| `aaa.<n>.radius.das.*`/`radius.dad.*`/`interim_update.*` rows | gated on `radius_das_enabled` (+ accounting) and `interim_update_enabled` | **split (2026-09-19 acct lane)**: `interim_update.*` implemented — `accounting_enabled` + `interim_update_enabled` → `interim_update.status=enabled`, `interim_update.interval=3600` (jar default; no interval knob). `das.*`/`dad.*` rows still omitted — the dad/das client rows' `<ip>` source is unrecovered from the jar and is not invented; `radius_das_enabled` is modeled on the profile but rejected at the admin API until the client rows are recoverable (renderer flags out-of-API carriers with an Alert) | the jar's own gates make das/dad unreachable without accounting_enabled anyway; the interim rows now ride that gate exactly, while the das/dad client source stays an open jar-recovery obligation |
