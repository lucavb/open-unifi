...
func TestBlockedClientsPersistAcrossReopen(t *testing.T) {
	...
	a := New(st, filepath.Join(t.TempDir(), "wireless.json"), filepath.Join(t.TempDir(), "site-settings.json"), SiteSettings{}, quietLogger())
	ctx := context.Background()
	...
	a2 := New(st2, filepath.Join(t.TempDir(), "wireless.json"), filepath.Join(t.TempDir(), "site-settings.json"), SiteSettings{}, quietLogger())
