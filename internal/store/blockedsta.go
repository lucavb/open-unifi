...
// Extra[BlockedStaExtraKey] — a sorted, deduplicated []any of canonical
...
// BlockedStaExtraKey is the Extra key of the admin-owned blocked-client set. The
// name matches the wire field the real controller's O00O writer produces
// (docs/PROTOCOL-mgmt.md §4).
const BlockedStaExtraKey = "blocked_sta"
...
func BlockedClients(d Device) []string {
	raw, _ := d.Extra[BlockedStaExtraKey].([]any)
...
func writeBlockedClients(d *Device, set []string) {
	...
	if len(set) == 0 {
		delete(d.Extra, BlockedStaExtraKey)
		return
	}
	...
	d.Extra[BlockedStaExtraKey] = raw
}
